...
[![CI](https://github.com/saigopalrathod/rag-chatbot/workflows/CI/badge.svg)](https://github.com/saigopalrathod/rag-chatbot/actions/workflows/ci.yaml)
...
>   * Ubuntu 22.04.2 LTS running on a Lenovo Legion 5 Pro with twenty 12th Gen Intel(R) Core(TM) i7-12700H and an NVIDIA GeForce RTX 3060.
...
> [!NOTE]
> The RecursiveCharacterTextSplitter class was refactored from LangChain to effectively chunk Markdown files without adding LangChain as a full dependency.
...
- Incremental ingestion pipeline: the pipeline diffs source docs against what's already indexed (using those version hashes). Only changed/new docs get processed.
...
> [!IMPORTANT]
> If you ever swap embedding models, you must rebuild the index from scratch since the vector spaces won't be compatible.
...
To easily install the dependencies and start the services, a makefile is provided.
...
In the .env we need to set the MODEL variable with the name of the model we want to load, and the MODEL_URL variable with the URL of the model in GGUF format:
```
...
> [!NOTE]
> We can load models that fit our hardware capacity and speed requirements.
> To decide which hardware to use/buy to host local LLMs we recommend reading these benchmarks:
...
> - Memory bandwidth mostly determines speed (tokens/sec).
> - If not, upgrade hardware or optimize the model.

We recommend starting with Qwen 3.5 9B or Meta Llama 3.2 Instruct 3B since they are small enough to run on a cheap GPU with 6GB of VRAM.

| Model | Model Size | Max Context Window | Notes |
|-------|------------|--------------------|-------|
| Qwen 3.6 27B | 27B | 262k | Recommended model |
| Qwen 3.6 35B A3B | 35B (3B activated) | 262k | |
| Qwen 3.5 0.8B | 0.8B | 256k | Tiny and fast multimodal |
| Qwen 3.5 2B | 2B | 256k | Multimodal for lightweight agents |
| Qwen 3.5 4B | 4B | 256k | Doesn't drift from tasks as bad as 2B |
| Qwen 3.5 9B | 9B | 256k | Recommended model |
| Meta Llama 3.2 Instruct | 1B | 128k | Optimized for mobile/edge |
| Meta Llama 3.2 Instruct | 3B | 128k | Optimized for mobile/edge |
| Meta Llama 3.1 Instruct | 8B | 128k | Old but still recommended |
| DeepSeek R1 Distill Qwen 7B | 7B | 128k | Experimental |

### Set the Embedding Model

For the semantic search, we support all the embedding models from Sentence Transformers.
...
We recommend using the jina-embeddings-v5-text models for multilingual retrieval tasks.
...
In the .env we need to set the SYNTHESIS_STRATEGY variable:
```
...
|-----------------------------|-----------|-------|
| create-and-refine | Yes | |
| tree-summarization | Yes | Recommended |


## Build the memory index

You can download Markdown pages and put them under docs.
...
Enable RAG Mode in the UI to ask questions based on the context provided by the indexed Markdown files:

![rag_chatbot_example.gif](images%2Frag_chatbot_example.gif)

You can also upload Markdown files using the file uploader. Once uploaded, they will be: uploaded -> chunked -> embedded -> upserted to Chroma.
...
This project is maintained by Sai Gopal Rathod, an AI/ML Engineer with over 4 years of experience building RAG pipelines, LLM-powered APIs, and distributed data systems. This implementation focuses on high-accuracy retrieval and efficient local LLM orchestration.
